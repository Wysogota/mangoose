...
const { coversOptions, defaultListOptions, relationshipsOptions } = require('./options');
...
module.exports.getMangaCovers = async (options) => {
  const query = queryString.stringify(coversOptions(options));
...
module.exports.getMangaList = async (options) => {
  ...
  const query = queryString.stringify(assignedOptions);
